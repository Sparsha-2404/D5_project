...
/** Maps client/proxy variants to Order schema enum values (strict match required). */
function normalizePaymentMethod(pm) {
  if (pm == null || pm === '') return 'cash_on_delivery';
  let s = String(pm).trim().toLowerCase();
  s = s.replace(/[\s-]+/g, '_');
  if (s === 'cod') return 'cash_on_delivery';
  const allowed = ['cash_on_delivery', 'wallet', 'simulated', 'card', 'razorpay'];
...
router.post('/', protect, async (req, res) => {
  try {
    ...
    const resolvedPayment = normalizePaymentMethod(paymentMethod);
    ...
    if (resolvedPayment === 'cash_on_delivery') {
